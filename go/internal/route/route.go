...
func HandlerCss(w http.ResponseWriter, _ *http.Request, path string) {
	w.Header().Set("Content-Type", "image/svg+xml")
	grid.RenderBase64(w, path)
}

// HandlerSvg returns the grid (text encoding).
func HandlerSvg(w http.ResponseWriter, _ *http.Request, path string) {
	w.Header().Set("Content-Type", "image/svg+xml")
	grid.Render(w, path)
}

// HandlerHtm returns an example (HTML page).
func HandlerHtm(w http.ResponseWriter, _ *http.Request, path string) {
