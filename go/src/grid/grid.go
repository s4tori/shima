...
// Render writes a valid SVG element into "w" using Base64 encodings.
...
func Render(w io.Writer, uri string) {
	...
	s.GBlurry()
	if c.XShow { s.Path(fmt.Sprintf("M 0 0 L %d 0", c.XCell), c.XDivC) }
	if c.YShow { s.Path(fmt.Sprintf("M 0 0 L 0 %d", c.YCell), c.YDivC) }
	s.Close("g")
	s.Close("pattern")
	...
	s.GBlurry()
	if c.XShow       { s.Path(fmt.Sprintf("M 0 0 L %d 0"  , c.XGrid), c.XGridC) }
	if c.YShow       { s.Path(fmt.Sprintf("M 0 0 L 0 %d"  , c.YGrid), c.YGridC) }
	if c.XGutterShow { s.Path(fmt.Sprintf("M 0 %d L %d %d", c.YGrid, c.XGrid, c.YGrid), c.XGridC) } // gutters
	if c.YGutterShow { s.Path(fmt.Sprintf("M %d 0 L %d %d", c.XGrid, c.XGrid, c.YGrid), c.YGridC) } // gutters
	s.Close("g")
	s.Close("pattern")
